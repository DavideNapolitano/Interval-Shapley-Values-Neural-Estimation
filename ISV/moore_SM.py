...
# OK
def calculate_grand_coalition(dataset, batch_size, num_workers, debug, dict_data, dim):
    ...
    for x in loader:
        ...
        for el in x:
            ...
            pred.append([(v_neg[0]+v_neg[1])/2, (v_pos[0]+v_pos[1])/2])
...
# OK
def generate_validation_data(val_set, imputer, validation_samples, sampler, batch_size, num_workers, dict_data, debug):
    # Generate coalitions.
...
def calculate_grand_inference(x, debug, dict_data, dim):
    if debug:
    ...
    v_neg=np.array([v1[0],v2[1]])
    v_pos=np.array([v2[0],v1[1]])
    grand.append([(v_neg[0]+v_neg[1])/2, (v_pos[0]+v_pos[1])/2])
...
class FastSHAP_M:
    ...
    def train(self,
        ...
        grand_train = calculate_grand_coalition(train_set, batch_size * num_samples, num_workers, False, train_dict_data, self.num_players).cpu()
        grand_val = calculate_grand_coalition(val_set, batch_size * num_samples, num_workers, False, val_dict_data, self.num_players).cpu()
        ...
        val_S, val_values = generate_validation_data(val_set, imputer, validation_samples, sampler, batch_size * num_samples, num_workers, val_dict_data, False)
        ...
        for epoch in range(max_epochs):
            ...
            for x, grand in batch_iter:
                ...
                for el, mask in zip(x_tiled,S):
                    ...
                    v_neg=np.array([v1[0],v2[1]])
                    v_pos=np.array([v2[0],v1[1]])
                    values.append([(v_neg[0]+v_neg[1])/2, (v_pos[0]+v_pos[1])/2])
    ...
    def shap_values(self, x, dict_mapping, debug):
        ...
        with torch.no_grad():
            ...
            grand = calculate_grand_inference(x, debug, dict_mapping, self.num_players)
